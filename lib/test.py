...
def runTransformer(target = "greeklish", datapath = "../dataset/pink_floyd", dummy_file = "dummy_target.txt"):
    ...
    ## I am doing this awfully inefficient iteration here because:
    ## a) I need the tokenizer before creating the dataset and I need to use build_from_corpus so I wont diverge from the original example
    ## b) Tensorflow just won't let me switch from lists to tf tensors and vice versa, so I'm  constructing every data type again and again and 
    ##    and somehow in the end I connect them all together
    for file in os.listdir(path):

        ## Fix the target dataset. One target output is one line of one song.
        with open(pt.join(path, file), 'r') as f:
            lines = f.readlines()
            lines = [x.replace("\n", "") for x in lines if x.replace("\n", "") != ""]
            random_lines += lines

        ## Gets inputs in tfDatasetAdapter format and pushes them to a temp dataset
        ## The dataset is temp because things are in string format. I don't have the tokenizer yet
        ## In the next file iteration, these values will be encoded with the tokenizer. What a mess!
    ...
    ## The script used to be unified rather than in functions,
    ## so global variables is a quick fix for that
    global tokenizer
    ...
    ## input is the tokenized line with a start and end token
    ## Output is a random line tokenized with a start and end token
    ...
    sample_decoder = Decoder(num_layers=2, d_model=512, num_heads=8, 
                                                     dff=2048, target_vocab_size=8500,
